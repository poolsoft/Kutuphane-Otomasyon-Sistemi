...
if( $s->kullanici($getUye,"uye_tur") == 1 ){
	$kime_gore = "5";
}elseif( $s->kullanici($getUye,"uye_tur") == 2 ){
	$kime_gore = "4";
}elseif( $s->kullanici($getUye,"uye_tur") == 3 ){
	$kime_gore = "3";
}else{
	$kime_gore = "3";
}
...
<div class="main-content">
	...
	<div class="hizala">
		...
		<h1 class="page-title"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Üye Düzenle</h1>

		<div class="page-content">
			
			<div class="uye-duzenle">
				<p class="firstp" style="margin-bottom:15px"><a href="?sayfa=uyeler">&laquo; Üyelere Dön</a></p>
				<?php
				if($getUye == $_SESSION["sp_user"]){
					echo "<h4 style='color:#C93131'>HATA! Kendinizi düzenleyemezsiniz.</h4>";
				}elseif( !$s->yetkisi_var($kime_gore,$s->kullanici($_SESSION["sp_user"],"uye_eylemler")) ){
					echo "<h4 style='color:#C93131'>HATA! Yetkiniz yok!</h4>";
				}elseif( !$s->uye_var_mi($getUye) ){
					echo "<h4 style='color:#C93131'>HATA! Böyle bir üye yok.</h4>";
				}else{
				?>
				<form method="POST" action="" id="updateUye" onsubmit="return false;" autocomplete="off" enctype="multipart/form-data">
				...
				<fieldset>
					<legend><?php echo htmlspecialchars($s->kullanici($getUye,"uye_adsoyad")); ?></legend>
			          ...
			          <?php }else{ //if bitiş ?>
			          <input type="hidden" name="a8" value="<?php echo $s->kullanici($getUye,"uye_tur"); ?>">
			          <?php } ?>
			          <p style="margin-bottom:10px"><label for="select_elementxx">Eylemler:</label>
			            	<div style="margin-top:-10px"><input type="checkbox" name="y1" id="a1" value="1"<?php if( $s->yetkisi_var("1",$yetkiler) ){ echo ' checked'; } ?>> <label for="a1">Eserleri düzenleyebilir.</label></div>
			            	<div><input type="checkbox" name="y2" id="a2" value="2"<?php if( !$s->yetkisi_var("2",$yetkilerx) ){ echo ' disabled'; } ?><?php if( $s->yetkisi_var("2",$yetkiler) ){ echo ' checked'; } ?>> <label for="a2">Teslimleri düzenleyebilir.</label></div>
			            	<div><input type="checkbox" name="y3" id="a3" value="3"<?php if( !$s->yetkisi_var("3",$yetkilerx) ){ echo ' disabled'; } ?><?php if( $s->yetkisi_var("3",$yetkiler) ){ echo ' checked'; } ?>> <label for="a3">Öğrencileri düzenleyebilir.</label></div>
			            	<div><input type="checkbox" name="y4" id="a5" value="4"<?php if( !$s->yetkisi_var("4",$yetkilerx) ){ echo ' disabled'; } ?><?php if( $s->yetkisi_var("4",$yetkiler) ){ echo ' checked'; } ?>> <label for="a5">Öğretmenleri düzenleyebilir.</label></div>
			            	<div><input type="checkbox" name="y5" id="a6" value="5"<?php if( !$s->yetkisi_var("5",$yetkilerx) ){ echo ' disabled'; } ?><?php if( $s->yetkisi_var("5",$yetkiler) ){ echo ' checked'; } ?>> <label for="a6">İdarecileri düzenleyebilir.</label></div>
			            	<div><input type="checkbox" name="y6" id="a4" value="6"<?php if( !$s->yetkisi_var("6",$yetkilerx) ){ echo ' disabled'; } ?><?php if( $s->yetkisi_var("6",$yetkiler) ){ echo ' checked'; } ?>> <label for="a4">Site ayarlarını düzenleyebilir.</label></div>
			            </p>
			 		  <div class="clearfix"></div>
			          <p><button class="button guncelle"><i class="fa fa-paper-plane" aria-hidden="true"></i>&nbsp; GÜNCELLE</button> <?php if($s->kullanici($_SESSION["sp_user"],"uye_tur")==1 || $s->kullanici($_SESSION["sp_user"],"uye_tur")==2 ){ ?>&nbsp; <button class="button delbut" onclick="uye_sil('<?php echo $getUye; ?>');return false;"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp; ÜYEYİ SİL</button><?php } ?></p>
				</fieldset>
				</form>
				<?php } ?>
			</div>
