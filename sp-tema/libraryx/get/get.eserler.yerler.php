...
if( !$s->giris_yapildi() ){
  ...
  exit();
	...
	$tum_yerler_cek = $s->query("SELECT eser_yer FROM eserler");
	$yerler = array();
	while($a = mysqli_fetch_assoc($tum_yerler_cek)){
		$yer = $a["eser_yer"];
		if(!in_array($yer, $yerler)){
			array_push($yerler, $yer);
		}else{
			continue;
		}
	}
...
<col style="width: 300px">
  ...
  <tr>
    <th class="tg-amwm">YER</th>
    <th class="tg-amwm">ESER</th>
  </tr>
  <?php
	foreach ($yerler as $yer) {
		$eserler_cek = $s->query("SELECT eser_ad,eser_yazar,eser_yayinevi,eser_isbn,eser_cat FROM eserler WHERE eser_yer='$yer' ORDER BY eser_cat DESC, eser_ad");
  ...
  ?>
  <tr>
    <td class="tg-baqh" style="font-size:25px;vertical-align:middle"><b><?php echo htmlspecialchars($yer); ?></b></td>
    <td class="tg-baqh"><b><?php echo htmlspecialchars($eser_ad); ?></b>
      ...
      <br>
      Kategori: <b><?php echo htmlspecialchars($cat_ad); ?></b>
    </td>
  </tr>
  <?php 
  		}
